...
class ArchiMateGenerator:
    ...
    def generate_online_url(self, plantuml_code: str) -> str:
        """Generate PlantUML online viewer URL with compatibility fixes."""
        try:
            # Create simplified PlantUML code without ArchiMate includes for online compatibility
            simplified_code = self._create_simplified_plantuml(plantuml_code)
            
            # Encode for PlantUML server
            import zlib
            import base64
            
            compressed = zlib.compress(simplified_code.encode('utf-8'), 9)
            encoded = base64.b64encode(compressed).decode('ascii')
            
            # Create PlantUML server URL
            plantuml_url = f"http://www.plantuml.com/plantuml/svg/{encoded}"
            
            return plantuml_url
            
        except Exception as e:
            return f"# Online URL generation failed: {str(e)}\n\n```plantuml\n{simplified_code if 'simplified_code' in locals() else plantuml_code}\n```"
    
    def _create_simplified_plantuml(self, plantuml_code: str) -> str:
        """Create simplified PlantUML code without ArchiMate includes for online compatibility."""
        ...
        for line in lines:
            ...
            # Skip ArchiMate include directives
            if line.startswith('!include') and 'archimate' in line.lower():
                continue
                
            # Convert ArchiMate elements to basic rectangles
            if any(archimate_element in line for archimate_element in [
                'Business_Actor', 'Business_Service', 'Business_Process', 'Business_Object',
                'Application_Component', 'Application_Service', 'Data_Object', 'Application_DataObject',
                'Technology_Node', 'Technology_Service', 'Technology_Device',
                'Motivation_Stakeholder', 'Motivation_Driver', 'Motivation_Goal', 'Motivation_Requirement',
                'Implementation_Work_Package', 'Workpackage'
            ]):
                # Convert to basic rectangle syntax
                simplified_line = self._convert_to_basic_rectangle(line)
                simplified_lines.append(simplified_line)
            elif line.startswith('Rel_'):
                # Convert ArchiMate relationships to basic arrows
                simplified_line = self._convert_to_basic_relationship(line)
                simplified_lines.append(simplified_line)
            else:
    ...
    def _convert_to_basic_rectangle(self, line: str) -> str:
        """Convert ArchiMate element to basic rectangle."""
        import re
        
        # Extract element info: ElementType(id, "name", color)
        match = re.match(r'([A-Za-z_]+)\(([^,]+),\s*"([^"]+)"(?:,\s*([^)]+))?\)', line)
        if match:
            element_type, element_id, element_name, color = match.groups()
            
            # Convert to basic rectangle with color
            color_part = f" #{color.strip()}" if color else " #lightblue"
            return f'rectangle "{element_name}" as {element_id}{color_part}'
        
        return line
    
    def _convert_to_basic_relationship(self, line: str) -> str:
        """Convert ArchiMate relationship to basic arrow."""
        import re
        
        # Extract relationship: Rel_Type(from, to, "label")
        match = re.match(r'Rel_([A-Za-z_]+)\(([^,]+),\s*([^,]+)(?:,\s*"([^"]+)")?\)', line)
        if match:
            rel_type, from_id, to_id, label = match.groups()
            ...
            arrow = arrow_styles.get(rel_type, arrow_styles['default'])
            label_part = f' : {label}' if label else ''
            
            return f'{from_id.strip()} {arrow} {to_id.strip()}{label_part}'
